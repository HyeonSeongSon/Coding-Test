...
> 어떤 과학자가 발표한 논문이 `n`편 중, `h`번 이상 인용된 논문이 `h`편 이상이고 나머지 논문이 `h`번 이하 인용되었다면, `h`의 최댓값이 이 과학자의 H-Index입니다.
...
그리고 나머지 `2`편의 논문은 `3`회 이하 인용되었기 때문에 이 과학자의 H-Index는 `3`입니다.
