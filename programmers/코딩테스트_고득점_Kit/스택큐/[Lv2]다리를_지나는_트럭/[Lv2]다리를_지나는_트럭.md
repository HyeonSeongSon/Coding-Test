...
예를 들어, 트럭 `2`대가 올라갈 수 있고 무게를 `10kg`까지 견디는 다리가 있습니다.
...
`solution` 함수의 매개변수로 다리에 올라갈 수 있는 트럭 수 `bridge_length`, 다리가 견딜 수 있는 무게 `weight`, 트럭별 무게 `truck_weights`가 주어집니다.
...
※ 공지 - 2020년 4월 6일 테스트케이스가 추가되었습니다.